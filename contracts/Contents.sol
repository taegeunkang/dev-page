...
contract Contents {
    ...
    mapping(address => string[]) private tagsOfWriter; // 마이페이지 사이드바 메뉴 
    ...
    modifier isContentOwner(uint256 _idx) {
        require(contentsOwnerLoc[_idx] == msg.sender, "msg.sender doesn't have this");
        _;
    }

    function getTagsOfWriter(address _writer) public view returns(Tags[] memory)  {
    ...
    // 게시글 생성
    function create(string memory _title,string memory _content, string[] memory _tags) external {
        contents.push(Content(contentsNumber, _title,_content, msg.sender, _tags, block.timestamp));
    ...
    // 게시글 수정(내용만 수정 가능)
    function edit(uint256 _idx, string memory _content) isContentOwner(_idx) public {
        contents[_idx].content = _content;
    }

    
    function getMycontentPageable(uint256 _current, uint256 _amount) public view returns(Content[] memory) {
        Content[] memory content = new Content[](_amount);
        uint256[] memory myContents = contentsOfowner[msg.sender];
        uint256 count = 0;

        for(uint256 i = _current; i < _safePagenation(_current, _amount, Page.Mypage); i++ ) {
    ...
    //페이지네이션 
    function getContentPageable(uint256 _current, uint256 _amount) public view returns(Content[] memory) {
        Content[] memory content = new Content[](_amount);
        uint256 count = 0;

        for(uint256 i = _current; i  < _safePagenation(_current, _amount, Page.Main); i++) {
    ...
    // 마지막 페이지 핸들링
    function _safePagenation(uint256 _cursor, uint256 _val, Page loc) private view returns (uint256) {
        uint256 size = (loc == Page.Main) ? contentsNumber : contentsOfowner[msg.sender].length;
